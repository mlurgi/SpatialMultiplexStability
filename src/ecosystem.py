...
from pylab import array, figure, ion, draw
...
from configure import MOVE_RADIUS, MOVE_RADIUS_TOP, MOVE_RADIUS_MUTUALISTS, CAPTURE_PROB
from configure import REPRODUCTION_RATE, SEXUAL_REPRODUCTION, ASEXUAL_REPRODUCTION_RATE_HIGH
...
class Ecosystem():
    ...
    def create_continous_habitats(self):
        ...
        cells_per_habitat = math.floor(float(ROWS*COLUMNS)/len(self.habitats))
...
#        draw()
    ...
    def apply_habitat_loss(self, type=HABITAT_LOSS_TYPE):
        cells_to_loose = math.floor((ROWS*COLUMNS)*LOST_HABITAT)
        lost_habitat = 0
        x_coord = self.rnd.randint(0,ROWS-1)
        y_coord = self.rnd.randint(0,COLUMNS-1)
        ...
        self.world[x_coord][y_coord].previous_habitat = self.world[x_coord][y_coord].habitat
...
#        #show()
    ...
    def recover_habitat(self):
        lost_habitat = 0
        for i in range(ROWS):
            for j in range(COLUMNS):
                if self.world[i][j].habitat == lost_habitat:
                    self.world[i][j].habitat = self.world[i][j].previous_habitat
        return
    ...
    def innoculate_world_with_species(self, species_id, fraction):
        max_inds = math.floor(ROWS*COLUMNS*fraction)
        sp_habitats = self.net.node[species_id]['habitats']
        ...
        for i in range(idx_row, ROWS):
            ...
            for j in range(idx_col, COLUMNS):
                ...
                if cur_inds >= max_inds:
                    return
                
                if current_cell.visitor == None and current_cell.habitat in sp_habitats and self.rnd.random() < fraction:
                    current_cell.visitor =  Individual(species_id)
                    self.new_inds_inmigration[species_id] += 1
                    cur_inds += 1
...
#                    return
    ...
    def _move_individual(self, i, j, visitor=False):
        ...
        if producer:
            ...
            reproduced = False
            if self.rnd.random() < SEXUAL_REPRODUCTION:
                reproduced = self.sexual_reproduction(current_indiv, i, j)
            else:
                if self.species_scl[current_indiv.species_id] == 3:
                    if self.rnd.random() < ASEXUAL_REPRODUCTION_RATE_HIGH:
                        reproduced = self.asexual_reproduction(current_indiv, i, j)
                else:
                    reproduced = self.asexual_reproduction(current_indiv, i, j)
        ...
        elif self.species_scl[current_indiv.species_id] == 3 :
            new_idx_x = self.rnd.randint(-MOVE_RADIUS_TOP, MOVE_RADIUS_TOP)
            new_idx_y = self.rnd.randint(-MOVE_RADIUS_TOP, MOVE_RADIUS_TOP)
        else:
    ...
    def asexual_reproduction(self, individual, i, j, present=10):
        ...
        #here we find the cell where the offspring is going to be located
        for x_offset in range(MATING_SPATIAL_RATIO*2):
            ...
            for y_offset in range(MATING_SPATIAL_RATIO*2):
                ...
                if mating_cell == None and temp_cell.visitor == None and temp_cell.habitat in self.net.node[individual.species_id]['habitats']:
                    mating_cell = temp_cell
                        
                #if mating_cell != None:
                 #   break    
                
                if ( (temp_cell.inhabitant != None and temp_cell.inhabitant.species_id == individual.species_id) ):
                    current_inds += 1
                    
                if ( (temp_cell.visitor != None and temp_cell.visitor.species_id == individual.species_id) ):
                    current_inds += 1
        
                if current_inds >= present:
                    return False
        ...
        if mating_cell != None:
            ...
            newborn.resource = (individual.reproduce(asexual=True))
...
#                        print 'visitor chosen for reproduction', partner.species_id
        ...
        if mating_cell != None and partner != None:
            ...
            if individual.species_id in self.mutualists:
                newborn.become_mutualist()
            newborn.resource = (individual.reproduce() + partner.reproduce())
    ...
    #this method applies a reduction of population of each of the species in the ecosystem reducing them to a fraction 'fraction'
    #of its current numbers...
    def apply_populations_reduction(self, fraction):
        self.count_individuals()
        max_inds = dict.fromkeys(self.species, 0)
        extinct = dict.fromkeys(self.species, 0)
        
        for sp in self.species:
            max_inds[sp] = int(math.ceil(self.populations[sp]*fraction))
        
        print 'I need to kill: ', max_inds.values()
        ...
        for i in range(idx_row, ROWS):
            ...
            for j in range(idx_col, COLUMNS):
                ...
                if current_cell.inhabitant is not None and type(current_cell.inhabitant) is not types.NoneType:
                    try:
                        inh_sp = current_cell.inhabitant.species_id
                        if self.rnd.random() < fraction and max_inds[inh_sp] > extinct[inh_sp]:
                            extinct[inh_sp] += 1
                            current_cell.inhabitant = None
                    except:
                        print 'inhabitant in cell ' + str(i) + ',' + str(j) + ' declared None but is:'
                        print type(current_cell.inhabitant)
                
                
                if current_cell.visitor is not None and type(current_cell.visitor) is not types.NoneType:
                    try:
                        vis_sp = current_cell.visitor.species_id
                        if self.rnd.random() < fraction and max_inds[vis_sp] > extinct[vis_sp]:
                            extinct[vis_sp] += 1
                            current_cell.visitor = None
                    except:
                        print 'visitor in cell ' + str(i) + ',' + str(j) + ' declared None but is:'
                ...
                if max_inds.values() == extinct.values():
                    print extinct.values()
                    return
    ...
    def remove_species(self, species_id):
        for i in range(ROWS):
            for j in range(COLUMNS):
                cell = self.world[i][j]
                if cell.inhabitant != None and cell.inhabitant.species_id == species_id:
                    cell.inhabitant = None
                    
                if cell.visitor != None and cell.visitor.species_id == species_id:
                    cell.visitor = None
