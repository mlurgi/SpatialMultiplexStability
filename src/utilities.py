...
def get_out_row(iteration, net, series_counts, offset, centroids, areas):
    ...
    if series_counts == '' or offset == 0:
        ...
        if (not (centroids is None)) and (not (areas is None)) and series_counts != '':
            space = True
            cvs_centroids = []
            cvs_areas = []
            cvs_densities = []
        ...
        for sp in net.nodes():
            ...
            for iterat in range(start,stop):
                ...
                if space and centroids.has_key(iterat) and len(centroids[iterat][sp]) == 2 and areas[iterat][sp] > 0:
...
def NetStats(writer, net, iteration, offset, series, calculate_is, out_dir, centroids=None, areas=None):
    ...
    if not(centroids is None) and not(areas is None):
        ...
        dict_stats = get_out_row(iteration, net, series_counts, offset, None, None)
